"use strict";
class HandComparer {
  // Board Cards!, players?, hands?, else?
  constructor(board) {
    this.tests = ["straight_flush", "four_of_a_kind", "full_house", "flush", "straight", "three_of_a_kind", "two_pair", "one_pair", "hi_card"];
    this.board = [board[1],board[2],board[3],board[5],board[7]];
  }
  // Functions
  // My functions (Interface Functions)
  getWinnerKeys(candidates) {
    console.log("Getting Winners!");
    // assemble my_players array compareable to players
    // +improve Add Split pots

    let my_players = [];
    for (let i=0; i<candidates.length;i++) {
      let player = {
        id: candidates[i].userId,
        carda: candidates[i].card1,
        cardb: candidates[i].card2,
      }
      my_players.push(player);
    }

    // get winners for my_players Array
  ...
  execute_test (string, player) {
    if (string === 'test_straight_flush') {
      return this.test_straight_flush(player);
    }
    if (string === 'test_four_of_a_kind') {
      return this.test_four_of_a_kind(player);
    }
    if (string === 'test_full_house') {
      return this.test_full_house(player);
    }
    if (string === 'test_flush') {
      return this.test_flush(player);
    }
    if (string === 'test_straight') {
      return this.test_straight(player);
    }
    if (string === 'test_three_of_a_kind') {
      return this.test_three_of_a_kind(player);
    }
    if (string === 'test_two_pair') {
      return this.test_two_pair(player);
    }
    if (string === 'test_one_pair') {
      return this.test_one_pair(player);
    }
    if (string === 'test_hi_card') {
      return this.test_hi_card(player);
    }
    alert("execute_test() cannot tokenize " + string);
  }

  execute_compare (string, hand_in, best_hand) {
    if (string === 'compare_straight_flush') {
      return this.compare_straight_flush(hand_in, best_hand);
    }
    if (string === 'compare_four_of_a_kind') {
      return this.compare_four_of_a_kind(hand_in, best_hand);
    }
    if (string === 'compare_full_house') {
      return this.compare_full_house(hand_in, best_hand);
    }
    if (string === 'compare_flush') {
      return this.compare_flush(hand_in, best_hand);
    }
    if (string === 'compare_straight') {
      return this.compare_straight(hand_in, best_hand);
    }
    if (string === 'compare_three_of_a_kind') {
      return this.compare_three_of_a_kind(hand_in, best_hand);
    }
    if (string === 'compare_two_pair') {
      return this.compare_two_pair(hand_in, best_hand);
    }
    if (string === 'compare_one_pair') {
      return this.compare_one_pair(hand_in, best_hand);
    }
    if (string === 'compare_hi_card') {
      return this.compare_hi_card(hand_in, best_hand);
    }
  ...
  winners_helper (my_players, test) {
    ...
    for (var i = 0; i < my_players.length; i++) {
      ...
      var a = this.execute_test("test_" + test, my_players[i]);
      ...
      if (typeof best === 'undefined') {
        ...
        var comp = this.execute_compare("compare_" + test, a, best);
  ...
  test_four_of_a_kind (player) {
    ...
    for (i = 0; i < my_cards.length; i++) {
      ranks[this.get_rank(my_cards[i]) - 2]++;
    }
    var four = "";
    var kicker = "";
    for (i = 0; i < 13; i++) {
    ...
    var num_mine = 0;
    if (this.get_rank(player.carda) == four) {
      num_mine++;
    }
    if (this.get_rank(player.cardb) == four) {
      num_mine++;
    }
    var num_needed = 4;
    if (four) {
  ...
  test_straight (player) {
    ...
    var num_mine = 0;
    for (i = 0; i < absolute_longest_stretch; i++) {
      if (absolute_hi_card - i == this.get_rank(player.carda) ||
          absolute_hi_card - i == this.get_rank(player.cardb)) {
        num_mine++;
      }
    }
    var hash_result = {};
  ...
  test_three_of_a_kind (player) {
    ...
    var num_mine = 0;
    if (this.get_rank(player.carda) == three) {
      num_mine++;
    }
    if (this.get_rank(player.cardb) == three) {
      num_mine++;
    }
    var num_needed = 3;
    if (three) {
  ...
  test_two_pair (player) {
    ...
    for (i = 0; i < my_cards.length; i++) ranks[this.get_rank(my_cards[i]) - 2]++;
    var first = "";
    var second = "";
    var kicker = "";
    for (i = 12; i > -1; i--) {
    ...
    var num_mine = 0;
    if (this.get_rank(player.carda) == first || this.get_rank(player.carda) == second) {
      num_mine++;
    }
    if (this.get_rank(player.cardb) == first || this.get_rank(player.cardb) == second) {
      num_mine++;
    }
    var num_needed = 2;
  ...
  test_hi_card (player) {
    ...
    for (i = 0; i < working_cards.length; i++) {
      working_cards[i] = this.get_rank(my_cards[i]);
    }
    for (i = 0; i < working_cards.length; i++) {
      if (working_cards[i] == null) {
        working_cards[i] = -1; // FF
